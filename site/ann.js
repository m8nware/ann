  ...
  if (i) {
    ...
    if (sel.getRangeAt(0).toString() === "") {
      ...
      $(anchor)
        ...
        .each(function(i, el) {
          if (!stop) {
            if (anchor == el) stop = true;
            else beg_off += $(el).text().length;
          }
        });
      ...
      $(focus)
        ...
        .each(function(i, el) {
          if (!stop) {
            if (focus == el) stop = true;
            else end_off += $(el).text().length;
          }
        });

      console.log(
        sel.anchorOffset + beg_off + " " + (sel.focusOffset + end_off)
      );
      var start = sel.anchorOffset + beg_off;
      var end = sel.focusOffset + end_off;
      if (start > end) {
        [start, end] = [end, start];
      }
      show_modal(false, start, end);
    }
  }
